...
//分布式模展开结果（属性一：CoordinateBlock(属性1：存放MatrixEntry的dataset，属性2,3：行数，列数),属性2 ：模展开的方向）
...
object DistributedTensor {
  ...
  private[hosvd] def computeSVD(
    ...
    val irm = (if (transposed) block.transpose else block).toIndexedRowMatrix//如果列数大于行数就转置
    ...
    val (uarr, urows, ucols) = if (extractU) {
      val arr = svd.U.rows.collect()//Array[Vector]
